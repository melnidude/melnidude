                                                              ...
                                                              double *A, int n)
{
   ...
   p_e = eigenvectors;
   memset( p_e, 0, n*n*sizeof(double) );
   for (i = 0; i < n; i++)
   {
	   *p_e = 1.0;
	   p_e += (n+1);
   ...
   for (pAk = A, k = 0; k < n; pAk += n, k++) eigenvalues[k] = fabs(*(pAk + k));
}
   ...
   while (threshold > threshold_norm) {
      ...
      for (pAk = A, k = 0; k < (n-1); pAk += n, k++) {
         for (pAm = pAk + n, m = k + 1; m < n; pAm += n, m++) {
            ...
            */
            for (p_e = eigenvectors, i = 0; i < n*n; p_e++, i += n) {
               dum1 = *(p_e + n*k);
               dum2 = *(p_e + n*m);
               *(p_e + n*k) = dum1 * cos_phi + dum2 * sin_phi;
               *(p_e + n*m) = - dum1 * sin_phi + dum2 * cos_phi;
            }
         }
         for (i = 0; i < n; i++)
            if ( i == k ) continue;
