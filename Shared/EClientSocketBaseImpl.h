...
void EClientSocketBase::placeOrder( OrderId id, const Contract &contract, const Order &order)
{
	...
	if( !m_connected) {
		OSTR_RESET
		ostr << "EClientSocketBase: not connected\n";
		OSTR_LOG(LOG_MSG);
	...
	if( m_serverVersion < MIN_SERVER_VER_UNDER_COMP) {
		OSTR_RESET
		ostr << "EClientSocketBase: MIN_SERVER_VER_UNDER_COMP\n";
		OSTR_LOG(LOG_MSG);
	...
	if( m_serverVersion < MIN_SERVER_VER_SCALE_ORDERS2) {
		OSTR_RESET
		ostr << "EClientSocketBase: MIN_SERVER_VER_SCALE_ORDERS2\n";
		OSTR_LOG(LOG_MSG);
	...
	if( m_serverVersion < MIN_SERVER_VER_ALGO_ORDERS) {
		OSTR_RESET
		ostr << "EClientSocketBase: MIN_SERVER_VER_ALGO_ORDERS\n";
		OSTR_LOG(LOG_MSG);

		if( !IsEmpty(order.algoStrategy)) {
			OSTR_RESET
			ostr << "EClientSocketBase: algoStrategy\n";
			OSTR_LOG(LOG_MSG);
	...
	if( m_serverVersion < MIN_SERVER_VER_NOT_HELD) {
		OSTR_RESET
		ostr << "EClientSocketBase: MIN_SERVER_VER_NOT_HELD\n";
		OSTR_LOG(LOG_MSG);
